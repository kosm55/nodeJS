...
class AuthMiddleware {
  ...
  public checkActionToken(type: ActionTokenTypeEnum, key = "token") {
    return async (req: Request, res: Response, next: NextFunction) => {
      try {
        // req.query[key]  і key = "token"  - це означає що по дефолту наш ключ має назву token,
        // якщо ми хочемо його назвати по іншому то у роутері в мідлвару ми можемо передати ще і назву яку матиме цей ключ
